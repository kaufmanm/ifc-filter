 ...
 */
public class LogisticForIFC extends AbstractClassifier implements OptionHandler {
...
private Enumeration attributes;
  ...
  protected double calculateLogLikelihood(double [][] X, double [] Y,
					  Matrix2 jacobian, double [] deltas) {
    ...
    for (int i = 0; i < X.length; i++) {
      ...
      // Update the log-likelihood of this set of coefficients
      //if( Y[i]==1 ) {
	LL = LL - 2 * Math.log(p)*Y[i];
      //} else {
	LL = LL - 2 * Math.log(1 - p)*(1-Y[i]);
      //}
  ...
  /**
   * Returns an enumeeration describing the available options
   *
   * @return an enumeeration of all the available options
   */
  ...
  public void buildClassifier(Instances train) throws Exception {
    ...
    this.train1 = new Instances(train);
    this.train1.delete();
    ...
    Matrix2 jacobian = new Matrix2(nR + 1, nR + 1);
  ...
  public double [] distributionForInstance(Instance instance)
    ...
    distribution[0] = evaluateProbability(instDat);
    distribution[1] = 1.0-distribution[1];
    return distribution;
  }
  ...
  public String toString() {
    ...
    for (int j = 1; j <= m_NumPredictors; j++) {
    	Attribute a = this.train1.attribute(j-1);
    	String attrtext = "";
    	if(a.name().contains("="))
    	{
    		attrtext += "case when " + a.name().replace("=", " in('") + "') then 1 else 0 end";
    		attrtext = attrtext.replace(",", "','");
    	}
    	else attrtext += a.name();
    	if(j>1) result += "+";
      result += Utils.doubleToString(m_Par[j], 12, 4) + "*" + attrtext
      + "\n";
    }
  ...
  public static void main(String [] argv) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new LogisticForIFC(), argv));
    } catch (Exception e) {
      e.printStackTrace();
